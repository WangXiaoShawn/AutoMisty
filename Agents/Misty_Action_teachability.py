...
class Teachability(AgentCapability):
    ...
    def __init__(
        ...
        self.Action_Agent_Task = None # task是从ActionAgent传递过来的 所以我们只需要解析它就能知道Task是什么
    ...
    def process_all_messages_before_reply(self, text: Union[dict, str, list]): # 这里的逻辑不对
        ...
        if isinstance(text, list):
            ...
            if self.Action_Agent_Task is None and tasks:
                self.Action_Agent_Task = tasks[0]  # 只赋值一次，后续不修改
            ...
            if self.Action_Agent_Task is None and text.get('name') == 'Draft_Action_Code':
                self.Action_Agent_Task = text.get('content', '')  # 只赋值一次
        else:
